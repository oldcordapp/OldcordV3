                    ...
                    <div className='mainPage-main'>
                        ...
                        <div className='mainPage-main-components' style={{
                            ...
                            {data && Array.isArray(data) && data.length > 0 ? <>
                                <Paginator header={`Audit Logs (Showing a maximum of 50 entries per page)`} tabs={['Action', 'Actioned By', 'Moderation ID', 'Timestamp', 'Reasoning', 'Moderated ID', 'Moderation Props']} maxPerPage={50}>
                                    {data.map((audit_log_entry, i) => (
                                        <AuditLog key={i} action={audit_log_entry.action} actioned_by={audit_log_entry.actioned_by} moderation_id={audit_log_entry.moderation_id} timestamp={audit_log_entry.timestamp} reasoning={audit_log_entry.reasoning} moderated_id={audit_log_entry.moderated.id} moderation_props={JSON.stringify(audit_log_entry.moderated)}></AuditLog>
                                    ))}
                                </Paginator>
