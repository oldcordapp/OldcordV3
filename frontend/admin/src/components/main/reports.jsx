    ...
    useEffect(() => {
        if (selectedId && data && Array.isArray(data)) {
            ...
            const report = data.find(
                (report) => String(report.id) === targetId
            );
                                        ...
                                        <div className='mainPage-main-components-sidebar'>
                                            {selectedReport != null ? <>
                                                ...
                                                <div className='mainPage-main-components-sidebar-infoLine'>
                                                    ...
                                                    <textarea readOnly={true} maxLength={1250} className="textarea-reports" value={selectedReport.subject} />
                                                </div>
                                                <div className='mainPage-main-components-sidebar-infoLine'>
                                                    <div className='mainPage-main-components-sidebar-label'>Description</div>
                                                    <textarea readOnly={true} maxLength={1250} className='textarea-reports' value={selectedReport.description} />
                                                </div>
                                                <div className='mainPage-main-components-sidebar-infoLine'>
                                                    <div className='mainPage-main-components-sidebar-label'>Email Address</div>
                                                    {selectedReport.email_address === null ? 'Not Provided' : selectedReport.email_address}
                                                </div>
                                                <div className='mainPage-main-components-sidebar-separator no-margin-override'></div>
                                                <div className='mainPage-main-components-sidebar-infoLine'>
                                                    ...
                                                    <div style={{
                                                        ...
                                                        <Button variant='danger' onClick={() => updateReport(selectedReport.id, 'discarded')} style={{
                                                            width: '100%',
                                                            fontFamily: 'Nebula Sans'
                                                        }}>Discard</Button>
                                                        <Button variant='success' onClick={() => updateReport(selectedReport.id, 'approved')} style={{
                                                            width: '100%',
                                                            fontFamily: 'Nebula Sans'
                                                        }}>Approve</Button>
                                                    </div>
                                        ...
                                        <div className='mainPage-main-components-main'>
                                            <div className='mainPage-main-components-wrapper'>
                                                {data && Array.isArray(data) && data.length > 0 ? <>
                                                    <Paginator header="Reports" tabs={['Id', 'Problem', 'Email Address', 'Actions']}>
                                                        {data.map((entry, i) => (
                                                            <Report key={i} id={entry.id} problem={entry.problem} email_address={entry.email_address ?? 'Not Provided'} onApprove={() => updateReport(entry.id, 'approved')} onDiscard={() => updateReport(entry.id, 'discarded')} selected={selectedReport && selectedReport.id === entry.id}></Report>
                                                        ))}
                                                    </Paginator>
                                                </> : <></>}
                                            </div>
