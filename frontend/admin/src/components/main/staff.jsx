...
const Staff = () => {
  ...
  useEffect(() => {
    if (selectedId && data && Array.isArray(data)) {
      ...
      const staff = data.find((staff) => String(staff.id) === targetId);
  ...
  const menuActions = [
    ...
    },
    {
      ...
      not_implemented_yet: selectedStaff && selectedStaff.staff_details.audit_log.length === 0,
      action: () =>
        setConfirmation({
          summary: `Are you sure you want to clear "${selectedStaff.username}"'s audit log?`,
          onYes: clearAuditLog,
        }),
    },
    {
      ...
      action: () =>
        setConfirmation({
          summary: `Are you sure you want to remove "${selectedStaff.username}" from staff?`,
          onYes: () => {
            removeFromStaff();
            closeConfirmation();
            window.location.reload();
  ...
  function getPrivilegeName(value) {
    return Object.keys(priviledgeFriendlyLabel).find(
      (key) => priviledgeFriendlyLabel[key] === value,
    );
  }
                  ...
                  {selectedStaff && (
                    <>
                      <div style={{ display: 'flex', alignItems: 'flex-start' }}>
                        ...
                        {selectedStaff.staff_details.audit_log &&
                        Array.isArray(selectedStaff.staff_details.audit_log) &&
