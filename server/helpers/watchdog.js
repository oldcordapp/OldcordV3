...
const Watchdog = {
    ...
    getFingerprint: async (protocol, headers, account = null, ja3hash = null) => {
        ...
        let presentHeaders = Object.keys(headers).map(Watchdog.normalizeHeader).filter(name => relevantHeaders.includes(name)).sort();
    ...
    middleware: (maxPerTimeFrame, timeFrame, sus_weight = 0.2, onTrip = null) => {
        ...
        return async function(req, res, next) {
            ...
            if (entry.count > maxPerTimeFrame) {
                ...
                logText(`Fingerprint: ${fingerprint} exceeded ${maxPerTimeFrame} reqs in ${timeFrame}ms from IP: ${req.ip}`, "watchdog");
                ...
                if (entry.sus_score > 7) {
                    ...
                    logText(`Fingerprint: ${fingerprint} is scoring high. Blocking them from proceeding for ~${block / 1000} seconds.`, "watchdog");
    ...
    setSusScoreDecay: (fingerprint) => {
        ...
        let decayTimer = setTimeout(() => {
            logText(`Clearing sus_score and entry for ${fingerprint} after ${Watchdog.susScoreDecayTime}ms.`, "watchdog");
